 ...
 * Copyright (c) 2022 zxkuhl and Megumin
 *
...
import { AboutIcon, BackupRestoreIcon, CloudIcon, DocsIcon, MainSettingsIcon, PaintbrushIcon, PatchHelperIcon, PlaceholderIcon, PluginsIcon, RMSTopSettingsIcon, UpdaterIcon } from "@components/Icons";
import { AboutTab, BackupAndRestoreTab, CloudTab, DocsTab, PatchHelperTab, PluginsTab, RMSTab, ThemesTab, UpdaterTab } from "@components/settings/tabs";
...
const settings = definePluginSettings({
    settingsLocation: {
        ...
        description: "Where to put the RMS settings section",
        options: [
...
export default definePlugin({
    ...
    buildLayout(originalLayoutBuilder: SettingsLayoutBuilder) {
        ...
        if (layout.some(s => s?.key === "rms_section")) return layout;

        const { buildEntry } = this;

        const rmsEntries: SettingsLayoutNode[] = [
            buildEntry({
                key: "rms_main",
                title: "RMS",
                panelTitle: "RMS Settings",
                Component: RMSTab,
            ...
            buildEntry({
                key: "rms_cloud",
                title: "Cloud",
                panelTitle: "RMS Cloud",
                Component: CloudTab,
                Icon: CloudIcon
            }),
            buildEntry({
                key: "rms_backup_restore",
            ...
            ...this.customSections.map((func, i) => {
                ...
                return buildEntry({
                    key: `rms_deprecated_custom_${section}`,
                    title: label,
                    Component: element,
                    Icon: section === "RMSTop" ? RMSTopSettingsIcon : PlaceholderIcon
                });
            })
        ].filter(isTruthy);

        const rmsSection: SettingsLayoutNode = {
            key: "rms_section",
            type: LayoutTypes.SECTION,
            useTitle: () => "RMS Settings",
            buildLayout: () => rmsEntries
        };
        ...
        layout.splice(idx, 0, rmsSection);
    ...
    get electronVersion() {
        return RMSNative.native.getVersions().electron || window.legcord?.electron || null;
    },

    get chromiumVersion() {
        try {
            return RMSNative.native.getVersions().chrome
    ...
    getInfoRows() {
        ...
        const rows = [`RMS ${gitHash}${additionalInfo}`];
