...
const makeRMSTopSwitcher = (branch: string) => () => {
    if (RMSTop.Settings.store.discordBranch === branch)
        throw new Error(`Already on ${branch}`);

    RMSTop.Settings.store.discordBranch = branch;
    RMSTopNative.app.relaunch();
};
...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => RMS.WebpackPatcher },
        wpInstances: { getter: () => RMS.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => RMS.Plugins },
        plugins: { getter: () => RMS.Plugins.plugins },
        Settings: { getter: () => RMS.Settings },
        Api: { getter: () => RMS.Api },
        Util: { getter: () => RMS.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("rms-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (RMS.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
        ...
        ...IS_RMSTOP ? {
            rmstopStable: makeRMSTopSwitcher("stable"),
            rmstopCanary: makeRMSTopSwitcher("canary"),
            rmstopPtb: makeRMSTopSwitcher("ptb"),
        } : {},
    };
}
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$rmsGetWrappedComponent) {
            return forceLoad ? value.$$rmsGetWrappedComponent() : value;
        }
...
export default definePlugin({
    ...
    start() {
        ...
        if (!IS_WEB) {
            const Native = RMSNative.pluginHelpers.ConsoleShortcuts as PluginNative<typeof import("./native")>;
