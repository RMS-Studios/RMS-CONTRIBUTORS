...
const settings = definePluginSettings({
    ...
    addBack: {
        ...
        // so disable this by default. RMSTop just doesn't, so we force enable it there
        hidden: IS_RMSTOP,
    }
});

const shouldAddBackMenus = () => IS_RMSTOP || settings.store.addBack;
...
export default definePlugin({
    ...
    patches: [
        // Add back Copy & Open Link
        {
            ...
            replacement: [
                ...
                {
                    match: /\i\.\i\.copy/,
                    replace: "RMS.Util.copyToClipboard"
                }
            ]
        },
    ...
    async copyImage(url: string) {
        ...
        if (IS_RMSTOP && RMSTopNative.clipboard) {
            RMSTopNative.clipboard.copyImage(await imageData.arrayBuffer(), url);
            return;
        } else {
